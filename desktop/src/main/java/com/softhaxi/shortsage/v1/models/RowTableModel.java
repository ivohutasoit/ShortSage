...
public class RowTableModel<T> extends AbstractTableModel {
   ...
   */
     ...
     * If <code>columnNames</code> does not have an entry fot this index 
    ...
    public boolean isCellEditable(int row, int column) {
    	...
    	return (isEditable == null) ? modelEditable : isEditable.booleanValue();
    }
    
    /**
     * Adds a row of data to the end of model
     * Notofication of the row being added will be generated.
     * 
     * @param rowData data of the row being edded.
     */
    public void addRow(T rowData) {
    	insert(get RowCount(), rowData);
    } 
    ...
    public List<T> getRowAsList(int... rows) {
    	ArrayList<T> rowData = new ArrayList<T>(row.lenght);
    ...
    public void insertRows(int row, T[] rowArray) {
    	...
    	for(int ii=0;ii<rowArray.length;ii++) 
    		rowList.add(rowArray[ii]);
     ...
     * 
     * This method will send a <code>tableRowsUpdated</code> notification message to all the listeners.
     * <p>
